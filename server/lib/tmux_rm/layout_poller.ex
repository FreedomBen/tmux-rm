defmodule TmuxRm.LayoutPoller do
  @moduledoc """
  ...
  def subscribe(session, window) do
    ...
    Phoenix.PubSub.subscribe(TmuxRm.PubSub, topic)
    ...
    case ensure_started(session, window) do
      ...
      {:error, reason} ->
        Phoenix.PubSub.unsubscribe(TmuxRm.PubSub, topic)
        {:error, reason}
    end
  end
  ...
  def unsubscribe(session, window) do
    Phoenix.PubSub.unsubscribe(TmuxRm.PubSub, topic(session, window))
  ...
  def start_link({session, window}) do
    name = {:via, Registry, {TmuxRm.PaneRegistry, {:layout_poller, session, window}}}
  ...
  defp lookup(session, window) do
    case Registry.lookup(TmuxRm.PaneRegistry, {:layout_poller, session, window}) do
      [{pid, _}] -> {:ok, pid}
      [] -> {:error, :not_found}
    end
  end
