defmodule TmuxRm.PaneStream do
  @moduledoc """
  ...
  def subscribe(target) do
    ...
    Phoenix.PubSub.subscribe(TmuxRm.PubSub, pubsub_topic)
    ...
    case ensure_started(target) do
      {:ok, pid} ->
        case GenServer.call(pid, {:subscribe, self()}) do
          ...
          {:error, reason} ->
            Phoenix.PubSub.unsubscribe(TmuxRm.PubSub, pubsub_topic)
            {:error, reason}
        end
  ...
  def start_link(target) do
    GenServer.start_link(__MODULE__, target,
      name: {:via, Registry, {TmuxRm.PaneRegistry, {:pane, target}}}
    )
  end
  ...
  def handle_call({:send_keys, data}, _from, state) do
    input_limit = Application.get_env(:tmux_rm, :input_size_limit, 131_072)
  ...
  def handle_info({port, {:data, bytes}}, %{pipe_port: port} = state) do
    max_bytes = Application.get_env(:tmux_rm, :output_coalesce_max_bytes, 32_768)
    coalesce_ms = Application.get_env(:tmux_rm, :output_coalesce_ms, 3)
  ...
  defp register_pane_id(pane_id) do
    case Registry.register(TmuxRm.PaneRegistry, {:pane_id, pane_id}, nil) do
      {:ok, _} -> :ok
  ...
  defp maybe_start_grace_period(state) do
    if MapSet.size(state.viewers) == 0 and state.grace_timer_ref == nil do
      grace_ms = Application.get_env(:tmux_rm, :pane_stream_grace_period, 30_000)
  ...
  defp broadcast(target, message) do
    Phoenix.PubSub.broadcast(TmuxRm.PubSub, "pane:#{target}", message)
  end
  ...
  defp ensure_started(target) do
    case lookup(target) do
      ...
      {:error, :not_found} ->
        case DynamicSupervisor.start_child(TmuxRm.PaneStreamSupervisor, {__MODULE__, target}) do
          {:ok, pid} -> {:ok, pid}
