defmodule TmuxRmWeb.TerminalChannelTest do
  ...
  describe "handle_in (direct module calls)" do
    test "resize with invalid bounds is a no-op" do
      ...
      assert {:noreply, ^socket} =
               TmuxRmWeb.TerminalChannel.handle_in(
                 "resize",
                 %{"cols" => 0, "rows" => 0},
                 socket
               )
    ...
    test "unknown events are handled gracefully" do
      ...
      assert {:noreply, ^socket} =
               TmuxRmWeb.TerminalChannel.handle_in("unknown_event", %{}, socket)
    end
  end
  ...
  describe "handle_info (direct module calls)" do
    ...
    test "unknown messages are handled gracefully" do
      ...
      assert {:noreply, ^socket} =
               TmuxRmWeb.TerminalChannel.handle_info(:some_random_message, socket)
    end
    ...
    test "DOWN with non-matching ref is ignored" do
      ...
      assert {:noreply, ^socket} =
               TmuxRmWeb.TerminalChannel.handle_info(
